...
insert into account
  (code, name, account_type_name, balance)
values
...
insert into account_setting
  (id, merchandise_inventory_account_code, cog_sold_account_code,
   sales_account_code, sales_discount_account_code,
   cash_over_and_short_account_code, default_purchase_payment_account_code,
...
(
  ...
  name               text    not null,
  category_name      text,
  cog_account_code   integer,                         -- historycal data applied from from account_setting
  cog                integer not null default 0,      -- cost of goods sold applied after account posting
  stock              integer not null default 0,      -- stock applied after account posting
  pending_stock_diff integer not null default 0,      -- stock applied instantly on pos
  unit_price         integer not null,
  uom                text    not null default 'unit', -- (unit of measurement) represent the smallest unit of the product
...
(
  ...
  alias           text,
  conversion_unit integer not null default 1, -- conversion unit to smallest unit of measurement (uom) of the product, it always be larger than or equal to the smallest unit.
...
(
  ...
  applied_amount integer not null, -- this is the final discount that will be applied to the sale
...
begin
  select
    case when new.min_amount > new.max_amount then raise(rollback,
...
create trigger account_mutation_trigger
  ...
  for each row
begin
  with mutation as (select
                      account.code              as code,
                      account.name              as name,
                      account.account_type_name as account_type_name,
                      account.balance           as balance,
                      case when account_type.increased_by = 'debit' then 1
                           else -1
                      end * new.amount          as mutation_amount
                    from account_mutation
                    left join account
                      on account.code = account_mutation.debit_account_code
                    left join account_type
                      on account_type.name = account.account_type_name
                    ...
                    select
                      ...
                      account.account_type_name as account_type_name,
                      account.balance           as balance,
                      case when account_type.increased_by = 'credit' then 1
                           else -1
                      end * new.amount          as mutation_amount
                    from account_mutation
                    left join account
                      on account.code = account_mutation.credit_account_code
                    left join account_type
                      on account_type.name = account.account_type_name
                    where account_mutation.id = new.id)
  insert
...
begin
  select
    case when new.account_post_time is not null then raise(rollback,
...
begin
  select
    case when old.account_post_time is not null then raise(rollback,
...
begin
  insert into journal_entry
    (entry_time, description)
  values
    (strftime('%s', 'now'), 'pembelian barang dari: ' || (select supplier.name
                                                          from supplier
                                                          where supplier.id = new.supplier_id));
  insert into account_mutation
    (journal_entry_id, debit_account_code, credit_account_code, amount)
  select
    last_insert_rowid(),
...
begin
  insert into product
    (id, name, unit_price, cog, stock, num_of_purchases)
  select
    product.id,
    product.name,
    product.unit_price,
  ...
  on conflict do update set
                          ...
                          num_of_purchases = product.num_of_purchases +
                                             count(purchase_item.purchase_id);
end;
...
begin
  insert into product
    (id, name, unit_price, pending_stock_diff)
  select
    product.id,
    product.name,
    product.unit_price,
...
begin
  insert into journal_entry
    (entry_time, description)
  values
    (strftime('%s', 'now'), 'pembukuan kasir oleh: ' || (select cashier.name
                                                         from cashier
  ...
  select
    ...
    case when product.stock <= 0 then
           ...
           case when product.stock < sum(sale_item.quantity) then
                  ...
                  product_account.balance * sum(sale_item.quantity) /
                  product.stock
           end
    end
  from sale_item
  ...
  left join product on product.id = sale_item.product_id
  left join account as product_account
    on product_account.code = product.cog_account_code
  left join account_setting on account_setting.id = 1
  ...
  from (select
          ...
          (psb.closing_balance - psb.opening_balance) -
          sum(sale_item.price)                                               as diff_balance
        from sale
        left join pos_session_balance psb
          on sale.pos_session_id = psb.pos_session_id and
             psb.account_code = sale.payment_account_code
        left join sale_item on sale_item.sale_id = sale.id
        where sale.pos_session_id = new.id
        group by sale.pos_session_id, sale.payment_account_code) as pos_balance
  left join account_setting on account_setting.id = 1
...
begin
  with sold as (select
                  ...
                  product.cog                                        as cog,
                  (product.cog * sold_item.quantity /
                   product.stock)                                    as cog_sold,
                ...
                from (select
                      ...
                      from sale_item
                      left join sale on sale.id = sale_item.sale_id
                      where sale.pos_session_id = new.id) as sold_item
                left join product on product.id = sold_item.product_id
                group by product.id)
  insert
  into product
    (id, name, unit_price, cog, stock, num_of_sales)
  select
    id,
    name,
    unit_price,
